...
const Daftar = () => {
  ...
  const isPassMeterStrong = () => {
    if (isPasswordMedium && isPasswordStrong) {
      ...
      return <span className="text-orange-500 text-[10px] sm:text-xs">Password Sedang, gunakan simbol untuk password yang kuat</span>;
    } else {
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={() => handleSubmit()}>
            <div className="mb-2">
              ...
              <input id="username" type="text" placeholder="Nama Lengkap" autoComplete="off" className={`input-primary ${isUsernameEmpty ? "border-red-500 focus:border-500" : "focus:border-accent"}'}`} aria-label="Username" value={username} onChange={(e) => setUsername(e.target.value)} />
            ...
            <div className="mb-2">
              ...
              <input id="email" type="email" placeholder="Contoh: johndee@gmail.com" autoComplete="off" className={`input-primary ${isEmailEmpty ? "border-red-500 focus:border-500" : "focus:border-accent"}`} aria-label="Email" required value={email} onChange={(e) => setEmail(e.target.value)} />
            ...
            <div className="mb-2">
              ...
              <div className="relative">
                <input id="password" type={showPassword ? "text" : "password"} onFocus={showPassMeter} onBlur={hidePassMeter} autoComplete="off" placeholder="Buat Password" className={`input-primary ${isPassEmpty ? "border-red-500 focus:border-500" : "focus:border-accent"}`} aria-label="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
                ...
                <div className={`text-xs transition-[height] duration-400 rounded-md ${passMeter ? "h-2 mt-2 mb-5" : "h-0"}`}>
                  <div className="w-full flex justify-between h-full">
                    <div className="bg-red-500 w-full rounded-s-md"></div>
                    <div className={`${isPasswordMedium && "bg-orange-500"} w-full`}></div>
                    <div className={`${isPasswordStrong && "bg-green-500"} w-full rounded-e-md`}></div>
                  </div>
                ...
                <div className="pass-check mt-2">
                  <div className={isMinPassLengthEight("style", password)}>
                    <span className="text-xs">
                      <FontAwesomeIcon icon={isMinPassLengthEight("icon", password)} /> Minimal panjang password 8 karakter
                    </span>
                  </div>
                  <div className={passWithNumAndLetter("style", password)}>
                    <div className="text-xs">
                      <FontAwesomeIcon icon={passWithNumAndLetter("icon", password)} /> Kombinasikan password dengan huruf dan angka
                    </div>
                  </div>
                </div>
              </div>
