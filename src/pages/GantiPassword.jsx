...
import { resetPassword, validateUser } from '../redux/actions/authAction'
...
const GantiPassword = () => {
  ...
  useEffect(() => {
    dispatch(validateUser(token, navigate))
  }, [])
  ...
  const handleSubmit = (e) => {
    ...
    checkEmptyFields()
    if (
      minPassLengthEight(newPassword) &&
      combineWithNumAndLetter(newPassword) &&
      confirmPassword === newPassword
    ) {
      setIsLoading(true)
      dispatch(resetPassword(token, newPassword, navigate)).then(() => setIsLoading(false))
    }
    setIsSubmitted(true)
  }
          ...
          <form>
            <div className="mb-2">
              ...
              <div className="relative">
                ...
                <p className={`${isNewPassEmpty ? 'block' : 'hidden'}  text-xs  text-red-500`}>
            ...
            <div>
              <ButtonPrimary onClick={handleSubmit} isDisabled={isLoading} text="Ubah Password" />
            </div>
          </form>
        </div>
        <Toast />
