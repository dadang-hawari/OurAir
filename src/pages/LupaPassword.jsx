    ...
    <div className="w-full flex h-screen justify-center items-center">
      <div className="flex items-center justify-center w-fit  relative h-fit m-auto px-5 pb-8 pt-14 border rounded-lg md:px-5 text-gray-900">
        <div className="w-full max-w-sm">
          <Link to="/login" className="absolute top-2 left-2 p-2 text-accent flex items-center gap-2">
          ...
          <p className="leading-5 text-xs">Kami akan mengirimkan Anda sebuah email berisikan link, yang digunakan untuk melakukan reset password.</p>
          <form onSubmit={handleSubmit}>
            <div className="my-6">
              <label htmlFor="email" className="text-left block mb-1 text-sm font-bold ">
                Email
              </label>
              <input id="email" type="email" placeholder="Masukkan Alamat Email Anda" className={`input-primary text-xs ${isEmailEmpty ? "border-red-500 focus:border-red-500" : "focus:border-blue-500"}`} value={email} aria-label="Email" onChange={(e) => setEmail(e.target.value)} />
              <p className={isEmailEmpty ? `text-xs block text-red-500` : `hidden`}>Mohon inputkan email anda</p>
            </div>
            <div>
