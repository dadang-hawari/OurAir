...
export const DepartureReturn = () => {
  ...
  const Close = () => {
    return (
      <div className="absolute top-0 cursor-pointer pt-2 py-1 px-4 right-0 text-xs text-gray-500" onClick={closeModal}>
        ...
        <div className="flex-grow">
          ...
          <div className="relative">
            ...
            <ReactModal isOpen={isModalOpen} onRequestClose={closeModal} style={customStyles} className="border-none absolute top-7 overflow-hidden">
              <Calendar className="rounded-xl px-5 pb-3 pt-5" value={departureReturn} onChange={handleDateChange} numberOfMonths={width < 650 ? 1 : 2} monthYearSeparator="-" range={isReturn} showOtherDays highlightToday={false} format="DD MMMM YYYY" weekDays={['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']} renderButton={renderButton}>
                <Close />
              </Calendar>
            </ReactModal>
          </div>
